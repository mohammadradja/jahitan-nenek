<header class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
    <div class="flex items-center gap-6">
        <button @click="sidebarOpen = !sidebarOpen" 
                class="w-12 h-12 rounded-xl border flex items-center justify-center transition-all shadow-sm"
                :class="sidebarOpen ? 'bg-white border-gray-100 text-dark-wool' : 'bg-dark-wool border-dark-wool text-white'">
    ...
    <div class="flex items-center space-x-6">
        ...
        <div class="flex items-center space-x-1 bg-white/50 p-1 rounded-xl border border-gray-100 shadow-sm">
            <a href="{{ route('lang.switch', 'id') }}" class="px-4 py-2 rounded-lg text-[10px] font-bold tracking-widest transition-all {{ App::getLocale() == 'id' ? 'bg-soft-rose text-white shadow-md' : 'text-gray-400 hover:text-dark-wool' }}">ID</a>
            <a href="{{ route('lang.switch', 'en') }}" class="px-4 py-2 rounded-lg text-[10px] font-bold tracking-widest transition-all {{ App::getLocale() == 'en' ? 'bg-soft-rose text-white shadow-md' : 'text-gray-400 hover:text-dark-wool' }}">EN</a>
        </div>
